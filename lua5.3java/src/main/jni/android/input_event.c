...
int init_uinput_dev() {
    ...
    ret = ioctl(uinput_fd, UI_SET_ABSBIT, ABS_MT_PRESSURE);
    if (ret) {
        ALOGE("ioctl fail:%s(%d)", __FUNCTION__, __LINE__);
        return -1;
    }
    ...
    ret = ioctl(uinput_fd, UI_SET_KEYBIT, BTN_TOOL_FINGER);
    if (ret) {
        ALOGE("ioctl fail:%s(%d)", __FUNCTION__, __LINE__);
        return -1;
    }
...
int touchMove(int type,int code, int val){
    if(type==0x00&&code==0x00&&val==0){
        int rd = write_event(type,code,val);
        sleep(40*1000);
        return rd;
    }
    return write_event(type,code,val);
}
...
int touchScroll(int x, int y,int pre){
    struct input_event event;
    int ret;
