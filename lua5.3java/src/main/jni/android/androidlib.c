 ...
 *
 * */
static int system_clicki(lua_State *L) {
    const char *text = lua_tostring(L, -1); /*获取参数X*/
    int result = input_click(text);
    if(result < 0)
...
static int system_touchMove(lua_State *L){
    int type = lua_tointeger(L, -3); /*获取参数X*/
    int code = lua_tointeger(L, -2); /*获取参数Y*/
    int val =  lua_tointeger(L, -1); /*获取参数Y*/

    int result = touchMove(type,code,val);
    if(result < 0)
        LOGE("error esystem_touchMove");
    return 0;
}
...
static int system_touchScroll(lua_State *L){
    int x = lua_tointeger(L, -3); /*获取参数X*/
    int y = lua_tointeger(L, -2); /*获取参数Y*/
    int pre = lua_tointeger(L, -1); /*获取参数Y*/

    int result = touchScroll(x, y,pre);
    if(result < 0)
